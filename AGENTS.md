...
- 版本发布：版本号同步维护于 `package.json` 与 `src-tauri/tauri.conf.json`（当前 4.6.9）；推送 `v*` tag 触发 GitHub Actions 自动构建 Windows 安装包并创建 GitHub Release；变更记录见 `CHANGELOG.md` / `CHANGELOG_EN.md`。
